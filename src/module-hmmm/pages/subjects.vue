        ...
        <el-col>
          <el-form :inline="true">
            <el-form-item label="学科名称">
              <el-input v-model="searchlist.subjectName"></el-input>
            </el-form-item>
      ...
      <template>
        <el-table :data="list" style="width: 100%">
          ...
          <el-table-column label="操作" fixed="right" width="230px">
            <template slot-scope="scope">
              <el-button size="small" type="text" @click="$router.push(`directorys?id=${scope.row.id}&name=${scope.row.subjectName}`)">学科分类</el-button>
              <el-button size="small" type="text" @click="$router.push(`tags?id=${scope.row.id}&name=${scope.row.subjectName}`)">学科标签</el-button>
...
export default {
  ...
  methods: {
    ...
    async getSubjectList () {
      const { data } = await getSubjectList(this.searchlist)
    ...
    // 清除搜索框
    clearSub () {
      this.searchlist.subjectName = ''
    },
    // 搜索功能
    searchSub () {
      // const searchlist = []
      // this.list.filter((item) => {
      //   if (item.subjectName.indexOf(this.key) !== -1) {
